...
called if either compilation or execution fails.  Multiple errors can
be generated in either phase, but in this example we're only
interested in the most recent error returned by B<KlelGetFirstError>.
We could elect to iteratively call B<KlelGetNextError> to get any
remaining errors.  Doing so is often useful for gathering more
information about the string of errors, if any, leading up to the most
recent error.  Note that errors are stored in the B<KLEL_CONTEXT>
structure on a LIFO basis.
