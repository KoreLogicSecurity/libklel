...
to retrieve error information using B<KlelGetError>.
...
will be present.  To retrieve this error, call B<KlelGetError>.  This
...
via the B<KlelGetError> function.
...
be retrieved using B<KlelGetCommandInterpreter> and
B<KlelGetCommandProgram>, respectively.  These functions return a
pointer to a string.  Note that these strings should not be freed.
